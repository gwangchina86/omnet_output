...
//    DropsV =  vector<vector<double>  > (100, vector<double>(100, 0));
    ...
    trafficlist = vector<vector<int>>(100, vector<int>(100,0));
...
void Statistic::getTrafficInfo(int flow_id, double *bandwidth, int *tsrc, int *tdest) {
            ...
            ifstream myfile (folderName + "/Traffic.txt");
                if(myfile.is_open()){
                    for(int i = 0; i < flow_num; i++){
                        ...
                        trafficlist[i][2] = dst;
//                        if(i==0) {
//                            cout<<"stat neib bw="<<*bandwidth<<",src"<<*src<<",dest="<<*dest<<endl;
//                        }
                        getline(myfile, aux,',');//Df
                    }
...
//       }
   ...
   for (int i = 0; i < flow_num; i++) {
//       for (int j = 0; j < numTx; j++) {
          long double datalength = BandPakcetsSize[i]/1024; // bandPacetsize 单位为bit，bandwidth单位为kbps
          //cout<<"bandwidth packets="<<BandPakcetsSize[i]<<",timet="<<timet<<endl;
          if (datalength == 0)
              features_bw.push_back(0);
          else
              features_bw.push_back((datalength)/(timet)); // 带宽= kbps, timet 为ms
